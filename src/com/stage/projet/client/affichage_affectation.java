...
public class affichage_affectation extends Composite implements HasText {

	private static affichage_affectationUiBinder uiBinder = GWT
			.create(affichage_affectationUiBinder.class);

	interface affichage_affectationUiBinder extends
			UiBinder<Widget, affichage_affectation> {
	}
...
greetingService.Afficher_Affectation_EnCours(new AsyncCallback<ListeAdresse>() {
			...
			public void onSuccess(ListeAdresse result) {
				...
				for(int i=0;i<n;i++)
				{
					...
					maTable.setText(i+1, 4,""+X.getV().get(i).getMaDD() );
					maTable.setText(i+1, 5,""+X.getV().get(i).getMaDF() );
	...
	@UiField Button Rechercher;

	public affichage_affectation(String firstName) {
		initWidget(uiBinder.createAndBindUi(this));
	...
	@UiHandler("Rechercher")
	void onRechercherClick(ClickEvent event) {
		...
		//Date DD=DateD.getValue();
		//Date DF=DateF.getValue();
		//String X1=DD.toString();
		//String Y1=DD.toString();
				
					greetingService.Rechercher_Affectation_EnCours(Nom, Prenom, Machine, "", "", c1, c2, c3, c4, new AsyncCallback<ListeAdresse>() {
						...
						public void onSuccess(ListeAdresse result) {
							...
							for(int i=0;i<=n;i++)
							{
								...
								maTable.setText(i+1,4, ""+B.getV().get(i).getMaDD() );
								maTable.setText(i+1,5, ""+B.getV().get(i).getMaDF() );
